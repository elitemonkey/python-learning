...
def proxy_handler(client_socket, remote_host, remote_port, recieve_first):
    ...
    if recieve_first:
        ...
        remote_buffer = response_handler(remote_buffer) # here we can modify the request, do fuzzing, test for authentication etc.
    ...
    # now lets loop and read from local, send to remote, send to local etc.
    while True:
        ...
        if len(remote_buffer):
            ...
            print "[<==] Sent to localhosst."
            
        # if no more data on either side, close the connections
        if not len(local_buffer) or not len(remote_buffer):
            client_socket.close()
            remote_socket.close()
            print "[*] No more daata. Closing connections."
